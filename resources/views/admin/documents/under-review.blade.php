...
<head>
    ...
    <title>Under Review | Admin Panel</title>
...
<body class="min-h-screen flex">
    ...
    <div class="flex-1 flex flex-col">
        ...
        <main class="p-8">
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-purple-300">🧐 Under Review</h1>
                <p class="text-purple-200 text-sm mt-2">Dokumen yang sedang dalam proses review</p>
            </div>
                    ...
                    <table class="w-full text-sm">
                        <thead class="text-purple-400 border-b border-white/10">
                            <tr>
                                ...
                                <th class="py-2 text-left">Review Level</th>
                                <th class="py-2 text-left">Deadline</th>
                                <th class="py-2 text-left">Days Left</th>
                                ...
                                <tr>
                                    ...
                                    <td class="py-3">
                                        <span class="px-2 py-1 bg-blue-600 rounded text-xs">
                                            Level {{ $item->current_reviewer_level }}
                                        </span>
                                    </td>
                                    <td class="py-3">{{ $item->tanggal_berlaku?->format('d M Y') ?? 'N/A' }}</td>
                                    <td class="py-3">
                                        @php
                                            $daysLeft = $item->tanggal_berlaku?->diffInDays(now());
                                            $color = $daysLeft && $daysLeft <= 3 ? 'text-red-400' : 'text-yellow-400';
                                        @endphp
                                        <span class="{{ $color }}">{{ $daysLeft ?? '-' }} days</span>
                                    </td>
                                    <td class="py-3">
