...
<head>
    <meta charset="UTF-8">
    <title>Review Masuk | Reviewer</title>
...
<main class="flex-1 p-6 space-y-6">
    ...
    <div class="glass rounded-xl p-6 flex justify-between items-center">
        <div>
            ...
            <p class="text-sm text-slate-400">
                Semua pengajuan yang tercatat di sistem
            </p>
        </div>
    ...
    <section class="glass rounded-xl overflow-hidden">
        ...
        <div class="grid grid-cols-12 gap-4 px-6 py-4 text-xs text-slate-400 border-b border-white/10">
            ...
            <div class="col-span-2">Pengaju</div>
        ...
        @forelse($pengajuanList as $p)
            <div class="grid grid-cols-12 gap-4 px-6 py-4 text-sm row-hover transition">
                ...
                <div class="col-span-2">
                    @php
                        $statusColor = match(strtolower($p->status)) {
                            'review1', 'review2', 'review3' => 'text-yellow-300',
                ...
                <div class="col-span-1 text-right">
                    @php
                        preg_match('/review(\d+)/i', $p->status, $match);
                        $statusLevel = $match[1] ?? null;
                    @endphp

                    @if($statusLevel && (int)$statusLevel === (int)auth()->user()->reviewer_level)
